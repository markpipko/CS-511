...
do_register(State, Ref, ClientPID, ClientNick) ->
    New_State = #chat_st{ name = State#chat_st.name,
...
%% Send message to each client in the chatroom 
send_to_clients(State, Ref, Message, CliNick, List_Of_Clients) ->
	case List_Of_Clients of 
		[H|T] -> H!{request, self(), Ref, {incoming_msg, CliNick, State#chat_st.name, Message}},
						send_to_clients(State, Ref, Message, CliNick, T);
		[] ->  [] end. 
...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	send_to_clients(State, Ref, Message, CliNick, maps:keys(maps:remove(ClientPID,State#chat_st.registrations))),
	New_State = #chat_st{ name = State#chat_st.name,
