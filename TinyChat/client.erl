...
do_join(State, Ref, ChatName) ->
	...
	false -> whereis(server)!{self(),Ref, join, ChatName},
		receive {ChatroomPID, Ref, connect, History} ->
			New_State = #cl_st{ gui = State#cl_st.gui,
				nick = State#cl_st.nick,
				con_ch = maps:put(ChatName, ChatroomPID, State#cl_st.con_ch)},
			{History, New_State}
		end
	end.
...
do_leave(State, Ref, ChatName) ->
	...
	true -> whereis(server)!{self(),Ref, leave, ChatName},
		receive {ChatroomPID, Ref, ack_leave} ->
			New_State = #cl_st{ gui = State#cl_st.gui,
				nick = State#cl_st.nick,
	...
	false -> whereis(server)!{self(),Ref, nick, NewNick},
		receive 
			{ServerPID,Ref,err_nick_used} -> 
				{err_nick_used, State};
			{ServerPID,Ref,ok_nick} -> New_State = #cl_st{ gui = State#cl_st.gui,
							nick = NewNick,
...
do_msg_send(State, Ref, ChatName, Message) ->
    	{ok, ChatroomPID} = maps:find(ChatName, State#cl_st.con_ch),
